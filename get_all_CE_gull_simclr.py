...
parser.add_argument('--resume', default='gull_checkpoint_100.tar', type=str, metavar='PATH',
...
def main():
    ...
    # data loader
    get_dataset = getattr(datasets_augmentation, "gull_simclr")
    ...
    model_fp = os.path.join('gull_checkpoint_100.tar')
    ...
    for i_remain in range(np.size(remaining_mask_size_pool)):
        ...
        for i in range(len(imlist)):
            ...
            if not os.path.exists(
                    "./gull/counterfactual_gradcam_simclr_mask/" + str(remaining_mask_size)[:4]):
                os.makedirs(
                    "./gull/counterfactual_gradcam_simclr_mask/" + str(remaining_mask_size)[:4])
            cv2.imwrite("./gull/counterfactual_gradcam_simclr_mask/" + str(remaining_mask_size)[:4] + '/' + img_name + '_groundtruth_' + str(int(target_class[i])) + '_counter_' + str(int(predicted_class[i])) + '_index_' + str(all_index[i]) + '.png', cur_mask)

            if not os.path.exists(
                    "./gull/counterfactual_gradcam_simclr_img/" + str(remaining_mask_size)[:4]):
                os.makedirs(
                    "./gull/counterfactual_gradcam_simclr_img/" + str(remaining_mask_size)[:4])
            cv2.imwrite("./gull/counterfactual_gradcam_simclr_img/" + str(
...
def cf_proposal_extraction2(val_loader, grad_cam_hp, grad_cam_cls, predicted_class, imlist, remaining_mask_size_pool, cross_match):
    ...
    for i_remain in range(np.size(remaining_mask_size_pool)):
        ...
        for i in range(len(imlist)):
            ...
            if not os.path.exists(
                    "./gull/demo_simclr/counterfactual_gradcam_mask/" + str(remaining_mask_size)[:4]):
                os.makedirs(
                    "./gull/demo_simclr/counterfactual_gradcam_mask/" + str(remaining_mask_size)[:4])
            cv2.imwrite("./gull/demo_simclr/counterfactual_gradcam_mask/" + str(remaining_mask_size)[
                                                                       :4] + '/' + img_name + '_groundtruth_' + str(
                int(target_class[i])) + '_counter_' + str(int(predicted_class[i])) + '_index_' + str(
                all_index[i]) + '.png', query_img)

            if not os.path.exists(
                    "./gull/demo_simclr/counterfactual_gradcam_img/" + str(remaining_mask_size)[:4]):
                os.makedirs(
                    "./gull/demo_simclr/counterfactual_gradcam_img/" + str(remaining_mask_size)[:4])
            cv2.imwrite("./gull/demo_simclr/counterfactual_gradcam_img/" + str(
