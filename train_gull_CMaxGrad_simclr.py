...
def main():
    ...
    teaching_set = './datasets/CUBgull/' + args.selection + '/CUBgull_Lt_gt_CE_simclr_tr.txt'
    remaining_set = './datasets/CUBgull/' + args.selection + '/CUBgull_Dt_gt_tr.txt'
    remaining_set_CE = './datasets/CUBgull/' + args.selection + '/CUBgull_Dt_gt_CE_simclr_tr.txt'
    ...
    for iter in range(args.iters):
        ...
        if iter == 0:
            ...
            counterlist_CE = []
            indexlist = []
            with open('./datasets/CUBgull/CUBgull_gt_CE_simclr.txt', 'r') as rf:
                for line in rf.readlines():
            ...
            assert callable(datasets.__dict__['gull_CE_simclr'])
            get_dataset = getattr(datasets, 'gull_CE_simclr')
            num_classes = datasets._NUM_CLASSES['gull_CE_simclr']
            ...
            assert callable(datasets.__dict__['gull_CE_simclr_Dt'])
            get_dataset = getattr(datasets, 'gull_CE_simclr_Dt')
            num_classes = datasets._NUM_CLASSES['gull_CE_simclr_Dt']
            train_loader_CE_Dt, val_loader_CE = get_dataset(
                batch_size=1, num_workers=args.workers, selection=args.selection)
            ...
            assert callable(datasets.__dict__['gull_CE_simclr_Lt'])
            get_dataset = getattr(datasets, 'gull_CE_simclr_Lt')
            num_classes = datasets._NUM_CLASSES['gull_CE_simclr_Lt']
    ...
    np.save('./gull/all_train_acc_iter_' + args.selection + '_imagenet_CE_simclr.npy', all_train_acc_iter)
    np.save('./gull/all_test_acc_iter_' + args.selection + '_imagenet_CE_simclr.npy', all_test_acc_iter)
