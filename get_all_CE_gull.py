...
parser = argparse.ArgumentParser(description='PyTorch end2end gull Training')
parser.add_argument('-d', '--dataset', default='gull_whole_s300', help='dataset name')
...
parser.add_argument('--resume', default='./gull/checkpoint_pretrain_resnet18_linear.pth.tar', type=str, metavar='PATH',
...
def main():
    ...
    linear_classifier = nn.Linear(512 * 1, num_classes, bias=False)
    checkpoint = torch.load(args.resume)
    linear_classifier.load_state_dict(checkpoint['state_dict_m'])
    linear_classifier = torch.nn.DataParallel(linear_classifier, device_ids=range(len(args.gpu))).cuda()
    ...
    # make gradcam
    grad_cam_cls = GradCam_cls(linear_classifier, model_main, target_layer_names=["layer4"], use_cuda=True)
    grad_cam_hp = GradCam_hp(linear_classifier, model_main, target_layer_names=["layer4"], use_cuda=True)


    # data loader
    assert callable(datasets.__dict__['gull_whole_s300'])
    get_dataset = getattr(datasets, 'gull_whole_s300')
    num_classes = datasets._NUM_CLASSES['gull_whole_s300']
...
class ModelOutputs_hp():
    ...
    def __init__(self, linear_model, model, target_layers):
...
class GradCam_hp:
    def __init__(self, linear_model, model, target_layer_names, use_cuda):
        self.model = model
        self.model.eval()
        self.cuda = use_cuda
        if self.cuda:
            self.model = model.cuda()
            self.linear_model = linear_model.cuda()

        self.extractor = ModelOutputs_hp(self.linear_model, self.model, target_layer_names)
...
class GradCam_cls:
    def __init__(self, linear_model, model, target_layer_names, use_cuda):
        ...
        self.extractor = ModelOutputs_cls(self.linear_model, self.model, target_layer_names)
    ...
    for i_remain in range(np.size(remaining_mask_size_pool)):
        ...
        for i in range(len(imlist)):
            ...
            if not os.path.exists(
                    "./gull/counterfactual_gradcam_mask/" + str(remaining_mask_size)[:4]):
                os.makedirs(
                    "./gull/counterfactual_gradcam_mask/" + str(remaining_mask_size)[:4])
            cv2.imwrite("./gull/counterfactual_gradcam_mask/" + str(remaining_mask_size)[:4] + '/' + img_name + '_groundtruth_' + str(int(target_class[i])) + '_counter_' + str(int(predicted_class[i])) + '_index_' + str(all_index[i]) + '.png', cur_mask)

            if not os.path.exists(
                    "./gull/counterfactual_gradcam_img/" + str(remaining_mask_size)[:4]):
                os.makedirs(
                    "./gull/counterfactual_gradcam_img/" + str(remaining_mask_size)[:4])
            cv2.imwrite("./gull/counterfactual_gradcam_img/" + str(
...
def cf_proposal_extraction2(val_loader, grad_cam_hp, grad_cam_cls, predicted_class, imlist, remaining_mask_size_pool, cross_match):
    ...
    for i_remain in range(np.size(remaining_mask_size_pool)):
        ...
        for i in range(len(imlist)):
            ...
            if not os.path.exists(
                    "./gull/demo/counterfactual_gradcam_mask/" + str(remaining_mask_size)[:4]):
                os.makedirs(
                    "./gull/demo/counterfactual_gradcam_mask/" + str(remaining_mask_size)[:4])
            cv2.imwrite("./gull/demo/counterfactual_gradcam_mask/" + str(remaining_mask_size)[
                                                                       :4] + '/' + img_name + '_groundtruth_' + str(
                int(target_class[i])) + '_counter_' + str(int(predicted_class[i])) + '_index_' + str(
                all_index[i]) + '.png', query_img)

            if not os.path.exists(
                    "./gull/demo/counterfactual_gradcam_img/" + str(remaining_mask_size)[:4]):
                os.makedirs(
                    "./gull/demo/counterfactual_gradcam_img/" + str(remaining_mask_size)[:4])
            cv2.imwrite("./gull/demo/counterfactual_gradcam_img/" + str(
