...
# two examples and two explanations for each iteration
...
def main():
    ...
    teaching_set = './datasets/butterflies_crop/' + args.selection + '/butterflies_Lt_gt_s300_CE_tr.txt'
    remaining_set = './datasets/butterflies_crop/' + args.selection + '/butterflies_Dt_gt_s300_tr.txt'
    remaining_set_CE = './datasets/butterflies_crop/' + args.selection + '/butterflies_Dt_gt_s300_CE_tr.txt'
    ...
    for iter in range(args.iters):
        ...
        if iter == 0:
            ...
            counterlist_CE = []
            indexlist = []
            with open('./datasets/butterflies_crop/butterflies_gt_s300_CE.txt', 'r') as rf:
                for line in rf.readlines():
            ...
            assert callable(datasets_augmentation.__dict__['butterflies_s300_wo_shuffle'])
            get_dataset = getattr(datasets_augmentation, 'butterflies_s300_wo_shuffle')
            num_classes = datasets_augmentation._NUM_CLASSES['butterflies_s300_wo_shuffle']
            train_loader, val_loader = get_dataset(
                batch_size=1, num_workers=args.workers)

            assert callable(datasets_augmentation.__dict__['butterflies_s300_CE'])
            get_dataset = getattr(datasets_augmentation, 'butterflies_s300_CE')
            num_classes = datasets_augmentation._NUM_CLASSES['butterflies_s300_CE']
            train_loader_CE_Dt, val_loader_CE = get_dataset(
                batch_size=1, num_workers=args.workers)

            if args.selection == 'MMT' or args.selection == 'MMT_largemargin':
                added_example_indices, groundtruth, counter = selection(train_loader, train_loader_CE_Dt, model_main, optimizer_m, iter, criterion,
            ...
            assert callable(datasets_augmentation.__dict__['butterflies_Dt'])
            get_dataset = getattr(datasets_augmentation, 'butterflies_Dt')
            num_classes = datasets_augmentation._NUM_CLASSES['butterflies_Dt']
            train_loader_Dt, val_loader = get_dataset(
                batch_size=1, num_workers=args.workers, selection=args.selection)

            assert callable(datasets_augmentation.__dict__['butterflies_s300_CE_Dt'])
            get_dataset = getattr(datasets_augmentation, 'butterflies_s300_CE_Dt')
            num_classes = datasets_augmentation._NUM_CLASSES['butterflies_s300_CE_Dt']
            train_loader_CE_Dt, val_loader_CE = get_dataset(
                batch_size=1, num_workers=args.workers, selection=args.selection)


            if args.selection == 'MMT' or args.selection == 'MMT_largemargin':
                added_example_indices, groundtruth, counter = selection(train_loader_Dt, train_loader_CE_Dt, model_main, optimizer_m, iter, criterion,
                                                  args.S_max, args.Lambda)
            elif args.selection == 'random':
            ...
            assert callable(datasets_augmentation.__dict__['butterflies_s300_CE_Lt'])
            get_dataset = getattr(datasets_augmentation, 'butterflies_s300_CE_Lt')
            num_classes = datasets_augmentation._NUM_CLASSES['butterflies_s300_CE_Lt']
    ...
    np.save('./butterflies/all_train_acc_iter_' + args.selection + '_data_augment_imagenet_CE.npy', all_train_acc_iter)
    np.save('./butterflies/all_test_acc_iter_' + args.selection + '_data_augment_imagenet_CE.npy', all_test_acc_iter)
...
def selection(train_loader, train_loader_CE, model_main, optimizer_m, epoch, criterion, S_max, Lambda):
    ...
    all_weights_numpy = np.array(all_weights)
    all_weights_CE_numpy = np.array(all_weights_CE)
    sum_weights = 0.5 * all_weights_numpy + 0.5 *all_weights_CE_numpy
    ...
    return [added_example_indices, added_counter_example_index], sorted_all_targets[0], counterfactual_class
